...
written. **60/60 automated tests pass** (21 backend, 13 frontend, 20 ai-service, 6
...
| Frontend dashboard (10 pages) | ✅ Build passes, 13/13 tests pass |
...
| AI service (real YOLOv8 detection; demo action/caption/weapon adapters) | ✅ 20/20 tests pass, real model verified live |
...
| Weapon/firearm detection | **Interface only, demo-labeled** | COCO has no firearm class; needs a custom-trained model — see [`docs/ai-pipeline.md`](docs/ai-pipeline.md) |
...
- **Weapon/firearm detection has no real model** — it's an interface + a mock adapter
  that always returns no detections. Building a real one needs a labeled firearm
  dataset and fine-tuning; see [`docs/ai-pipeline.md`](docs/ai-pipeline.md).
